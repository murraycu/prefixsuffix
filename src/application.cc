...
Application::Application()
  : Gtk::Application(PREFIXSUFFIX_APP_ID, Gio::APPLICATION_HANDLES_COMMAND_LINE)
  , m_window(nullptr)
...
#define PREFIXSUFFIX_GLADE_RESOURCE_PATH                                       \
  "/io/github/murraycu/PrefixSuffix/data/"
...
Application::create_window()
{
  ...
  try {
    builder = Gtk::Builder::create_from_resource(
      PREFIXSUFFIX_GLADE_RESOURCE_PATH "prefixsuffix.glade");
  } catch (const Gtk::BuilderError& ex) {
    std::cerr << G_STRFUNC << ": BuilderError Exception: " << ex.what()
              << std::endl;
  } catch (const Glib::MarkupError& ex) {
    std::cerr << G_STRFUNC << ": MarkupError exception:" << ex.what()
              << std::endl;
  } catch (const Glib::FileError& ex) {
    std::cerr << G_STRFUNC << ": FileError: exception: " << ex.what()
              << std::endl;
  } catch (const Gio::ResourceError& ex) {
    std::cerr << G_STRFUNC << ": Gio::ResourceError: exception: " << ex.what()
              << std::endl;
  } catch (const Glib::Error& ex) {
    std::cerr << G_STRFUNC << ": Exception of unexpected type: " << ex.what()
              << std::endl;
  }
  ...
  const Glib::RefPtr<Glib::VariantDict>& options)
{
  ...
  if (version) {
    const Glib::ustring msg = Glib::ustring::compose(
      _("%1 - Version %2"), Glib::get_application_name(), PACKAGE_VERSION);
