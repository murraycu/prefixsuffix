...
FileRenamer::FileRenamer(const std::string& directory_path,
  ...
  bool recurse_into_folders, bool operate_on_folders, bool operate_on_hidden)
...
FileRenamer::start()
{
  ...
  /*
  if( m_prefix_replace.empty() && m_prefix_with.empty() &&
  m_suffix_replace.empty() && m_suffix_with.empty() )
  {
    std::cerr << G_STRFUNC << ": prefix and suffix strings are empty." <<
  std::endl;
    return;
  }
*/
...
void
FileRenamer::on_set_display_name(const Glib::RefPtr<Gio::AsyncResult>& result,
  const Glib::RefPtr<Gio::File>& file)
{
  try {
    const Glib::RefPtr<const Gio::File> file_renamed =
      file->set_display_name_finish(result);
    if (!file_renamed) {
      std::cerr << G_STRFUNC
                << ": null result from Gio::File::set_display_name_finish()."
                << std::endl;
    ...
    std::cerr << G_STRFUNC
              << ": Exception from Gio::File::set_display_name_finish(): "
              << ex.what() << std::endl;
...
FileRenamer::rename_next_file()
{
  ...
  file->set_display_name_async(uriNew,
    sigc::bind(sigc::mem_fun(*this, &FileRenamer::on_set_display_name), file),
    m_cancellable);
}

void
FileRenamer::do_rename_files()
{
  if (m_files.empty()) {
    stop_process(
      _("No files have this prefix or suffix, so no files will be renamed."));
    return;
  }
...
void
FileRenamer::build_list_of_files()
{
...
void
FileRenamer::request_next_files(const Glib::RefPtr<Gio::File>& directory,
  const Glib::RefPtr<Gio::FileEnumerator>& enumerator)
{
  enumerator->next_files_async(
    sigc::bind(sigc::mem_fun(*this, &FileRenamer::on_directory_next_files),
      directory, enumerator),
    m_cancellable, 5 /* number to request at once */);
}

void
FileRenamer::on_directory_next_files(
  ...
  const Glib::RefPtr<Gio::FileEnumerator>& enumerator)
{
  ...
  try {
    ...
    for (auto info : list_info) {
      ...
      if (basename.empty()) {
        std::cerr << G_STRFUNC
                  << ": child->get_basename() return an empty string."
                  << std::endl;
        stop_process(
          _("PrefixSuffix failed while obtaining the list of files."));
        continue;
      }
      ...
      if (use) {
        ...
        if (file_type == Gio::FILE_TYPE_DIRECTORY) {
          ...
          const std::string& basename_new =
            m_string_renamer.get_new_basename(basename);
          if (basename_new.empty()) {
            std::cerr << G_STRFUNC << ": m_string_renamer.get_new_basename("
                      << basename << ") returned an empty string." << std::endl;
            m_string_renamer.debug_cout();
            stop_process(
              _("PrefixSuffix failed while building the new filename."));
            continue;
          }

          if (basename_new !=
              basename) // Ignore it if the prefix/suffix change had no effect
          {
            m_files.push(uri);
    ...
    std::cerr << G_STRFUNC
              << ": Exception with directory uri=" << directory->get_uri()
              << ": " << ex.what() << std::endl;
  ...
  for (const auto& child_dir : list_folders) {
    ...
    if (m_operate_on_folders) {
      ...
      if (child_dir !=
          filepath_new) // Ignore it if the prefix/suffix change had no effect
      {
...
void
FileRenamer::on_directory_enumerate_children(
  const Glib::RefPtr<Gio::AsyncResult>& result,
  const Glib::RefPtr<Gio::File>& directory)
{
  try {
...
void
FileRenamer::build_list_of_files(const std::string& directorypath_uri_in)
{
  // This is a recursion.
  ...
  m_directory_enumerations_in_progress.insert(
    m_directory_enumerations_in_progress.end(), directory);
  directory->enumerate_children_async(
    sigc::bind(
      sigc::mem_fun(*this, &FileRenamer::on_directory_enumerate_children),
      directory),
    m_cancellable, G_FILE_ATTRIBUTE_STANDARD_NAME
...
void
FileRenamer::get_folder_and_file(
  const std::string& filepath, std::string& folderpath, std::string& filename)
{
...
void
FileRenamer::stop_process(const Glib::ustring& error_message)
{
  if (m_cancellable) {
...
FileRenamer::type_signal_stopped
FileRenamer::signal_stopped()
{
  return m_signal_stopped;
}

FileRenamer::type_signal_progress
FileRenamer::signal_progress()
{
